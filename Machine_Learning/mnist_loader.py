...
def load_mnist():
    ...
    np_train_images = np.matrix(train_images, dtype="float64").transpose() / 255
    np_train_labels = np.asmatrix(np.zeros((10, len(train_labels)), dtype=int))
    for index, num in enumerate(train_labels):
        np_train_labels[num, index] = 1

    np_test_images = np.matrix(test_images, dtype="float64").transpose() / 255
    np_test_labels = np.asmatrix(np.zeros((10, len(test_labels)), dtype=int))
