...
class VideoCapture(object):
    def __init__(self, channel: int, rate_limit=None, convert_network=False):
        self.pipe = None
    ...
    def update_m3u8_file(self):
        ...
        if text != self.last_m3u8:
            ...
            for file_name in text.splitlines():
                if ".ts" in file_name and not os.path.exists(os.path.join(self.PATH_TO_TS_FILES, file_name)):
                    file_url = self.cap_url[:-10] + file_name
                    data = requests.get(file_url).content
                    with open(os.path.join(self.PATH_TO_TS_FILES, file_name), mode="wb") as f:
                        f.write(data)
