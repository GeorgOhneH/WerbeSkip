...
Aus dem Grund kann das Logo komplett herausfiltert werden und wieder verwenden wenn man das Logo mit einem schwarzen Hintergrund findet (siehe Abbildung~\ref{fig:logo5}f).
...
Mit diesen selber generierten Bildern kann ein neuronales Netzwerk trainiert werden, welches das Logo erkennen kann.
...
Ausserdem enthält der Datensatz noch 400 Bilder, die ein spezielles Prosieben Logo auf dem Bild haben (siehe Abbildung~\ref{fig:logo_spec})
...
\begin{figure}[h]%
    ...
    \caption{Speziellen Prosieben Logo}%
    \label{fig:logo_spec}%
\end{figure}
\subsection{Methoden}
...
Wenn man die genaue Architektur von einem Netzwerk bestimmen will, ist das oft nicht ganz einfach,
...
Schicht wird RelU angewendet.
Maxpooling wird weniger häufig angewendet, da es Heute nicht mehr so üblich ist.\cite{conv}
Es werden relativ kleine Werte für Dropout genommen, da genug Daten vorhanden sind und dadurch overfitting kein so grosses Problem darstellt.
Ausserdem wird in jeder Schicht Batch Normalization verwedent, nach dem Beispiel von ResNet.\cite{conv}

Die letzte Schicht beinhaltet 2 Neuronen und die softmax Aktivierungsfunktion.
Der erste steht für kein Logo, wobei 0 für Falsch steht und 1 für Wahr.
...
und am Schluss muss man langsam in das Minimum hineinkommt.
...
sondern die \textit{cross-entropy}\cite{softmax}, welche mit dem gleichen Prinzip funktioniert wie die Quadratische, mit dem Unterschied,
dass mit der softmax Aktivierungsfunktion sich viel mathematisch kürzt und die implementierung vereinfacht\cite{softmax}.
...
Auf Abbildung~\ref{fig:loss1}a,b wird der loss und der MCC,
des Netzwerk im Verlaufe des Trainings, dabei ist zu beachten,
dass der Mittelwert genommen wurde, so dass auf der Abbildung noch etwas zu erkennen ist, da die Werte sonst zu extrem wären.
\begin{figure}[h]%
    \centering
...
Wenn man 71'800 selbst generierte Bilder mithilfe des neuronalen Netzwerkes kategorisiert erhält man durchschnittich einen loss von 0.139,
...
Der Datensatz schneidet beim MCC um 8\% schlechter ab, als der selbst generierten Bilder.
Das könnte daran liegen, dass im Fernsehen oft noch neben Logo auch andere Einblendungen haben (siehe Abbildung~\ref{fig:einblenden}).
\begin{figure}[h]%
    \centering
    \includegraphics[width=7cm]{assets/images/tv_einblenden.png}%
...
da es manchmal unmöglich ist das Logo zu erkennen.
Genau das ist ein grosses Problem bei einem neuronalen Netzwerk.
Man hat keine Ahnung was das Netzwerk wirklich macht und wie es genau funktioniert.
...
Das ist ziemlich überraschend, da das Netzwerk diese Art von Logo noch nie gesehen hat und trotzdem so gut abschneidet.
Das spezielle Logo ist komplett Weiss und ein bisschen kleiner als das Normale.
Man könnte Meinen, dass das Netzwerk nicht unbedingt das Logo erkennt,
...
Dieser Wert ist 15\% schlechter als wenn das normale Logo mit generierten Bildern verwendet wird.
Das zeigt sehr schön, dass nicht nur die Helligkeit der Pixel entscheidend ist sondern auch die Form.
...
Wenn die Vorhersagen des Netzwerks in einem Zeitraum in einem Diagramm grafisch dargestellt wird (siehe Abbildung)
