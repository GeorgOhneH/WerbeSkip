...
Die Schichten speichern selber, die Daten, die sie für die backpropagation brauchen.
...
Jede Schicht bekommt einen Optimierer, um die Gewichte und Biases anzupassen.
\paragraph{Netzwerk}
...
Die GPU ist so viel schneller als die CPU, da alles mithilfe von matrix Multiplikationen implementiert ist und
...
Der Ordner enthält die Funktionen für die Formatierung der Bilder, damit sie vom Netzwerk benutzt werden können.
Um die Bilder zu bearbeiten wird OpenCV 2 und Numpy verwendet.

Die grösste Herausforderung war die Beschaffung der live Bilder für einem Sender.
...
Um das fertige Programm auch zu benutzen wird eine Webseite verwendet.
...
so dass die Verbindung mit dem Server immer offen ist und die Information ob Werbung läuft sofort übermittelt werden kann.
...
Aus den 7830 Bildern sind 4495 Bilder mit Logo und ohne Rand, 813 mit Logo und einem schwarzen Rand oben und unten,
...
\paragraph{Treffsicherheit} Eine andere relativ einfache Methode ist die Treffsicherheit des Netzwerks zu bestimmen,
indem man die richtig erkannten Bilder durch die totale Anzahl Bilder teilt.

\paragraph{Matthews correlation coefficient} Das Problem
...
Abbildung aus dem Kapitel~\ref{ch:neuronalesNetzwerk} sind von Michael A. Nielsen Buch\cite{neuralbook} inspiriert.
\listoffigures
